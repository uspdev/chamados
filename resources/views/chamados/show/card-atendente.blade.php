...
<div class="card bg-light border-warning mb-3" id="card-atendente">
  ...
  <div class="card-body">
    <div class="row">
      ...
      <div class="col-md-4">
        @includewhen(Gate::check('atendente') && $chamado->status != 'Fechado', 'chamados.show.mudar-status')
        @includewhen(Gate::check('atendente') && $chamado->status == 'Fechado', 'chamados.show.formulario-form-atendente')
        @includewhen(Gate::check('atendente') && $chamado->status != 'Fechado', 'chamados.show.formulario-dados-atendente')
      </div>
    </div>
  </div>
</div>
  ...
  <script>
    $(function() {
      ...
      $anotacoes.keyup(function() { // keypress or keyup to detect backspaces
        $status.show()
        $status.attr('class', 'badge badge-warning text-light').html(
          '<i class="fas fa-sync-alt fa-spin"></i> Pendente');
          ...
          $.ajax({
            ...
            success: function(data) {
              ...
              if (data.message == 'success') {
                $status.attr('class', 'badge badge-secondary').html(
